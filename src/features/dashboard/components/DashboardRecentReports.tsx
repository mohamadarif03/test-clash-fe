    ...
    return (
        <div className="bg-white rounded-xl shadow-card overflow-hidden">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
                <div>
                    <h3 className="text-lg font-bold text-gray-900">Laporan Soal Terbaru</h3>
                    <p className="text-sm text-gray-500 mt-1">Soal yang dilaporkan oleh siswa untuk ditinjau.</p>
                </div>
                <button className="text-sm font-medium text-primary hover:text-primary-dark hover:underline cursor-pointer">Lihat Semua</button>
            </div>
                            ...
                            <tr key={report.id} className="hover:bg-gray-50/50 transition-colors group">
                                ...
                                <td className="px-6 py-4">
                                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${report.statusColor}`}>
                                        <span className={`size-1.5 rounded-full ${report.status === 'Menunggu' ? 'bg-orange-600' : report.status === 'Selesai' ? 'bg-green-600' : 'bg-red-600'}`}></span>
                                        {report.status}
                                    </span>
                                </td>
